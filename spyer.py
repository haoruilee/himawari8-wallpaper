...
font_path = os.path.join(base_path, "fonts", "times.ttf")
earth_img_path = os.path.join(base_path, "latest-earth.jpg")
wallpaper_img_path = os.path.join(base_path, "wallpaper.jpg")
...
formatted_time = yesterday - timedelta(
    minutes=yesterday.minute % 10,  # 对齐到10分钟间隔
...
def download_earth_img():
    ...
    for i in range(slices):
        for j in range(slices):
            ...
            temp_img_path = os.path.join(base_path, f"temp_{i}_{j}.png")
...
def generate_wallpaper():
    ...
    try:
        ...
        print(f"Failed to save wallpaper to {wallpaper_img_path}: {e}")
...
if __name__ == '__main__':
    ...
    if os.path.exists(wallpaper_img_path):
        ...
        print("Failed to apply wallpaper. No image found at", wallpaper_img_path)
